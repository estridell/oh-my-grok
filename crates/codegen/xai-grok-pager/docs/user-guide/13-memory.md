...
Memory lets OMG recall facts, decisions, and patterns from earlier sessions. OMG indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each OMG session starts fresh: the model knows nothing about previous sessions. When you enable memory, OMG can:
...
|----------|-------|-------------|
| `~/.oh-my-grok/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.oh-my-grok/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.oh-my-grok/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

OMG suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, OMG saves a structured metadata summary to that session's daily log. The summary contains:
...
OMG builds the summary from conversation metadata without an LLM call, without added latency. OMG skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask OMG to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.oh-my-grok/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

OMG records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, OMG opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, OMG shows `Memory saved to ~/.oh-my-grok/memory/MEMORY.md`.

### Forget

Ask OMG to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.oh-my-grok/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what OMG remembers:

```
> what do you remember?
```

OMG searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.oh-my-grok/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, OMG confirms in the scrollback:

```
Memory saved to ~/.oh-my-grok/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what OMG has stored.
...
Dream also runs automatically. By default, OMG checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, OMG automatically searches memory for content relevant to the current project and injects it as context. This means OMG starts with knowledge from previous sessions without a reminder.
...
OMG searches memory automatically, but you can also trigger searches manually in the chat:
...
omg memory clear --workspace

# Clear the global MEMORY.md
omg memory clear --global

# Clear both workspace and global memory
omg memory clear --all

# Skip the confirmation prompt (-y is the short form)
omg memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.oh-my-grok/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.oh-my-grok/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, OMG applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, OMG uses the session's primary model. |
...
When a session memory is old, OMG attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, OMG watches `~/.oh-my-grok/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
```bash
ls ~/.oh-my-grok/memory/
cat ~/.oh-my-grok/memory/MEMORY.md
$EDITOR ~/.oh-my-grok/memory/MEMORY.md
```

### Debug Logging

```bash
RUST_LOG=debug GROK_LOG_FILE=/tmp/omg.log omg
grep "memory" /tmp/omg.log
```
