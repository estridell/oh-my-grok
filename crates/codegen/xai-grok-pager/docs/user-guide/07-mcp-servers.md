...
MCP (Model Context Protocol) servers extend OMG with external tool integrations. They let OMG interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to OMG over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside OMG's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.oh-my-grok/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

OMG spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; OMG-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.oh-my-grok/config.toml`) **or repo-level**
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. OMG handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers OMG's own OAuth client with the provider.

OMG walks from the current directory up to the git repo root, loading `.grok/config.toml` at each level:
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. OMG reads most other config sections only from `~/.oh-my-grok/config.toml`.
...
You can enable or disable MCP servers during a session without restarting OMG.
...
OMG loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native OMG config | `~/.oh-my-grok/config.toml`, `.grok/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.oh-my-grok/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `omg inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, OMG handles the credential flow automatically. When an MCP server requests OAuth credentials, OMG opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. OMG stores the resulting tokens under `~/.oh-my-grok/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). OMG expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, OMG captures the process's standard error to `~/.oh-my-grok/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.oh-my-grok/logs/mcp/filesystem.stderr.log
```
...
```bash
omg inspect          # Human-readable
omg inspect --json   # Machine-readable
```
