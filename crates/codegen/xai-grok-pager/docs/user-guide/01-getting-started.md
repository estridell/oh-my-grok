...
Use `/login xai` for the stock X account flow or `/login chatgpt` for ChatGPT
OAuth. Credentials and other user state are stored under `~/.oh-my-grok`.
...
Every conversation is a **session**. Sessions are automatically saved to `~/.oh-my-grok/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `omg -c`
...
# Experimental scrollback-native render mode. Sticky: plain `omg` reopens in
...
```bash
omg -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
