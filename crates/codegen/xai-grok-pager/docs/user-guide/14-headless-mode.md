...
Headless mode runs OMG non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
OMG processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, OMG emits an error object (process exit non-zero). Prompt-level
...
OMG may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `omg -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
omg -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
omg -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
omg -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
omg -p "What's the secret number?" --resume abc123
```
...
omg -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
omg --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  omg -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
OMG's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(omg -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
omg -p "Format all files" --yolo

# Run tests and fix failures
omg -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
- **`omg login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`omg login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `omg -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug omg -p "..." 2> debug.log`.

---

## Project Root Discovery

When OMG starts, it discovers the project root by walking upward from `--cwd`
...
OMG discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
OMG stores data in `~/.oh-my-grok` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
### Read-Only `~/.oh-my-grok`

For containers or CI, mount `~/.oh-my-grok` read-only:
...
unless it runs from the managed install (`~/.oh-my-grok/bin/omg`, or the
equivalent configured home).
...
- Resume: `omg -p "continue" --resume "<id>"` or `omg -p "continue" --continue`
