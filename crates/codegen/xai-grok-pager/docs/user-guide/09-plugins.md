...
If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, OMG discovers the components above from their standard directories.
...
```bash
omg plugin list [--json] [--available]   # List installed plugins (--available requires --json)
omg plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
omg plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
omg plugin update [<name>]               # Omit the name to update all plugins
omg plugin enable <name>
omg plugin disable <name>
omg plugin details <name>                # Show the plugin's component inventory
omg plugin validate [<path>]             # Validate plugin.json (default: current directory)
omg plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `omg plugin install <source>` without `--trust` and OMG prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
omg plugin marketplace list [--json]
omg plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
omg plugin marketplace remove <url>      # Git URL or local path of a configured source
omg plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
omg plugin marketplace add my-org/team-plugins
omg plugin marketplace list
omg plugin install my-org/team-plugins --trust
omg plugin list
omg plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `omg plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.oh-my-grok/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `omg plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.oh-my-grok/pager.toml`:
...
Place this file at `~/.oh-my-grok/settings.json` or `~/.claude/settings.json`.
...
OMG trusts plugins from `~/.oh-my-grok/plugins/` automatically. Project plugins in `.grok/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
omg plugin install <source> --trust
```
...
Run `omg inspect` to see every discovered plugin and what it provides:

```bash
omg inspect          # Show plugins with their skills, agents, hooks, and MCP servers
omg inspect --json   # Emit machine-readable JSON
```
