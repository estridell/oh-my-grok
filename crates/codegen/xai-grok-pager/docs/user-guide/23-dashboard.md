...
- **`omg dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.oh-my-grok/config.toml` under `[keybindings]` like every other shortcut.
...
```
 OMG · Dashboard — 4 agents · 2 awaiting
...
The state icon matches OMG's sibling views (
`tasks_pane`):
...
rebound via `~/.oh-my-grok/config.toml`.
...
process. The plan's Phase 4 ("supervisor / `omg --bg`") would list
