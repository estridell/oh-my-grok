...
Agent mode runs OMG as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`omg -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `omg agent` command and apply to every mode. Pass them before the mode name, for example `omg agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
...
```bash
omg agent serve --bind 127.0.0.1:2419 --secret <token>
```
...
Beyond the base ACP protocol, OMG defines extension methods under the `x.ai/` prefix for SpaceXAI-specific functionality. These cover:
