...
Hooks let you run custom scripts or HTTP requests at key moments during an OMG session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.oh-my-grok/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.oh-my-grok/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.oh-my-grok/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match OMG's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
OMG injects the following variables into every hook process:
...
2. The current process environment (the env OMG itself sees).
...
Hooks from `~/.oh-my-grok/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.oh-my-grok/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.oh-my-grok/logs`).
...
Copy them to `~/.oh-my-grok/hooks/` and customize.
