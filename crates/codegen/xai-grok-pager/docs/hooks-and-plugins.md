...
OMG supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.oh-my-grok/hooks/`
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.oh-my-grok/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `omg plugin marketplace add <source>`)
...
Sources land in `~/.oh-my-grok/config.toml`:
...
Or in `~/.oh-my-grok/settings.json` / `~/.claude/settings.json`:
