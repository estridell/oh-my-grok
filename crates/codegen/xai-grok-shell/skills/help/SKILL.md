...
name: help
description: >
  OMG documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any OMG feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "OMG docs — config, MCP, auth, skills, commands"
---

# OMG Help

Answer the user's question about OMG setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.oh-my-grok/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.oh-my-grok/docs/user-guide/`. The available guides are:
   ...
   `~/.oh-my-grok/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.oh-my-grok/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.oh-my-grok/skills/<name>/SKILL.md`
   (read `~/.oh-my-grok/docs/user-guide/08-skills.md` for the SKILL.md format).