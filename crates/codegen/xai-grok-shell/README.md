...
- **Reference**
  - [Introspection (`omg inspect`)](#introspection)
...
Credentials are stored under `~/.oh-my-grok` and persist across sessions.
...
**3. Run `omg`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.oh-my-grok/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`omg -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
```bash
omg --debug-file /tmp/grok-auth.log -p "hello"
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.oh-my-grok/auth.json)" \
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.oh-my-grok/auth.json` (set by `omg login`)                                                                                                                              |
...
> **Note:** `auth.json` tokens expire after 7 days. Run `omg login` to refresh.
...
omg -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
omg -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
omg -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
omg -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
omg -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
omg -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
omg -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
omg -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
omg -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
omg --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
omg -p "What does this project do?"

# Use a specific model
omg -p "Optimize this function" -m grok-build

# Get JSON output for parsing
omg -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
omg -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
omg -p "Remember: the secret number is 42" --output-format json
omg -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
omg -p "Continue where we left off" -c

# Run in a different directory
omg -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
omg -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
omg -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
omg -p "Generate a README" > README.md

# Parse JSON output with jq
omg -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
omg -p "Review changes for bugs and security issues." \
...
for file in src/*.js; do
  omg -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
omg -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
```bash
omg agent headless --grok-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`omg ssh`)

Use `omg ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
omg ssh -t user@host
omg ssh -L 8080:localhost:8080 user@host

# With remote command
omg ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `omg ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Use headless mode for simple integrations. Spawns `omg -p` and parses JSON output.
...
With this config, `omg` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.
...
| `~/.oh-my-grok/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.oh-my-grok/skills/`) are outside the repo and never filtered.
...
**Combining with `[endpoints]` config:** You can also set endpoints in `~/.oh-my-grok/config.toml`:
...
MCP servers are configured in `~/.oh-my-grok/config.toml`:
...
| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.oh-my-grok/config.toml`           | All projects      | Lowest   |
...
If you also have a `linear` server in `~/.oh-my-grok/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.grok/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.oh-my-grok/config.toml`.
...
Memory is stored as Markdown files under `~/.oh-my-grok/memory/`:
- **Global** (`~/.oh-my-grok/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.oh-my-grok/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.oh-my-grok/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.oh-my-grok/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.oh-my-grok/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.oh-my-grok/memory/` for external edits and reindex on search |
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.oh-my-grok/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.oh-my-grok/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.oh-my-grok/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.oh-my-grok/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.oh-my-grok/sandbox.toml` (global) or `.grok/sandbox.toml` (per-project):
...
Sandbox events (profile applied, violations) are logged to `~/.oh-my-grok/sandbox-events.jsonl`
...
omg inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.grok/skills/`, `~/.oh-my-grok/skills/`, plugins, and config paths
...
Sessions are stored under `~/.oh-my-grok/sessions/`, organized by URL-encoded working directory:

```
~/.oh-my-grok/sessions/<encoded-cwd>/<session-id>/
...
omg -p "Remember: X=42" -s my-session
omg -p "What is X?" -s my-session

# Resume existing session (errors if not found)
omg -p "Continue" -r my-session

# Continue most recent session in current directory
omg -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
omg -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.oh-my-grok/config.toml` | Configuration file                                  |
| `~/.oh-my-grok/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.oh-my-grok/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.oh-my-grok/memory/`     | Cross-session memory files and index                |
| `~/.oh-my-grok/skills/`     | User-scoped skill definitions                       |
| `~/.oh-my-grok/plugins/`    | User-scoped plugins                                 |
| `~/.oh-my-grok/agents/`     | User-scoped agent definitions                       |
...
| `OH_MY_GROK_HOME`              | Override the config and state directory (default: `~/.oh-my-grok`)                                            |
| `GROK_HOME`                    | Legacy alias for `OH_MY_GROK_HOME`                                                                            |
...
| `GROK_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.oh-my-grok/debug/<sessionId>.txt`, a path writes that one file |
...
Generate completions for your shell and install them to enable tab completion for `omg` commands and flags.
...
omg completions zsh > ~/.zsh/completions/_grok
```
...
omg completions zsh > ~/.oh-my-grok/completions/zsh/_grok
```
...
Regenerate completions after upgrading `omg` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `omg --debug` or `GROK_LOG_FILE` instead:

```bash
# Per-session debug log (~/.oh-my-grok/debug/<sessionId>.txt)
omg --debug
...
ls ~/.oh-my-grok/sessions/

# Read session metadata
cat ~/.oh-my-grok/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.oh-my-grok/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.oh-my-grok/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
