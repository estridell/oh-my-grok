...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   OMG agent server starting...");
    eprintln!();
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(xai_grok_telemetry::otel_layer::build_otel_layer(
            xai_grok_telemetry::otel_layer::OtelClientInfo {
                ...
                client_version: xai_grok_version::XAI_PROTOCOL_VERSION,
    ...
    xai_grok_telemetry::external::init(
        xai_grok_shell::agent::config::resolve_external_otel_config(
            xai_grok_telemetry::external::config::ExternalClientInfo {
                ...
                client_version: xai_grok_version::XAI_PROTOCOL_VERSION.to_owned(),
...
async fn run_setup_command(json: bool) {
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!("  omg setup");
        eprintln!();
        eprintln!("Or add the key to ~/.oh-my-grok/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's deployment administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    if !use_leader {
        anyhow::bail!(
            "`grok workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.oh-my-grok/config.toml, or pass --leader."
        );
    }
    ...
    if !xai_grok_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.oh-my-grok/config.toml and \
...
fn main() {
    ...
    if let Err(e) = xai_grok_config::validate_requirements() {
        eprintln!("Couldn't start oh-my-grok: {e}");
        eprintln!();
        eprintln!(
            "Update OMG to a version the policy allows, or ask your administrator \
    ...
    if xai_grok_shell::util::config::load_crash_handler_enabled_sync() {
        ...
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("OMG crashed during your last session.");
...
/// Handle `omg update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_omg_target() {
        let home =
        ...
        let target = home.join("downloads").join("omg-1.2.3");
